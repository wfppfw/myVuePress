...
//cursor
// const defaultUrl = ref(themeLocale.value.curDefault) ;
// const hoverUrl = ref(themeLocale.value.curHover);
// navbar
...
onMounted(() => {
  ...
  document.querySelector('body')!.style.cursor  = themeLocale.value.curDefault;
  // let length = document.querySelectorAll('a')
  for(let i = 0;i<document.querySelectorAll('a').length;i++){
    document.querySelectorAll('a')[i]!.style.cursor  = themeLocale.value.curHover;
  }

  window.onblur = function () {
    (<any>window).realTitle = document.title
    document.title = themeLocale.value.outTitle
    // document.title = '我在这里' + document.title
  }
  window.onfocus = function () {
    document.title = (<any>window).realTitle ? (<any>window).realTitle : themeLocale.value.inTitle
  }
