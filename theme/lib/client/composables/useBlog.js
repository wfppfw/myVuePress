...
export const blogsSymbol = Symbol.for("blogs");
...
export const setupBlogPages = () => {
    const blogs = useBlogType("post");
    provide(blogsSymbol, blogs);
};
export const useBlog = (pageIndex) => {
    ...
    const blogNumPerPage = computed(() => themeLocale.value.blogNumPerPage);
    const maxPageIndex = computed(() => Math.max(1, Math.ceil(posts.value.length / blogNumPerPage.value)));
    const pageValidIndex = computed(() => {
        if (!pageIndex)
            return -1;
        return Math.max(1, Math.min(pageIndex.value, maxPageIndex.value));
    });
    const postIndex = computed(() => posts.value.findIndex((item) => item.path === router.currentRoute.value.path));
    const post = computed(() => postIndex.value === -1 ? null : posts.value[postIndex.value]);
    ...
    const postListPager = computed(() => {
        if (!pageIndex)
            return {};
        const next = pageValidIndex.value > 1
            ? Math.min(pageIndex.value, maxPageIndex.value) - 1 === 1
                ? "/"
                : `/page/${pageIndex.value - 1}`
            : null;
        const prev = pageValidIndex.value < maxPageIndex.value
            ? `/page/${pageIndex.value + 1}`
            : null;
        return {
            next,
